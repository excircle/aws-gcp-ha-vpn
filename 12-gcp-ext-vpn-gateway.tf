...
resource "google_compute_external_vpn_gateway" "external_gateway" {
  ...
  name            = "aws-${aws_ec2_transit_gateway.aws_to_gcp_tgw.id}-${data.aws_region.current.name}-akalaj"
  redundancy_type = "FOUR_IPS_REDUNDANCY"
  description     = "AWS Transit GW: ${aws_ec2_transit_gateway.aws_to_gcp_tgw.id} in AWS region ${data.aws_region.current.name}"
